...
let timerState = {
    ...
    vmixSettings: {},
    clockMode: false,
    currentLanguage: 'el', // Προεπιλογή: Ελληνικά
    timelineSettings: {
        startTime: '09:00',
        endTime: '17:00'
    },
        ...
        if (timerState.isRunning) {
            ...
            if (timerState.timeLeft <= timerState.warningThreshold && timerState.timeLeft > 0) {
                ...
                if (!timerState.flashSentForEnd) {
                    // ΚΡΙΣΙΜΟ: Χρησιμοποίησε τη γλώσσα που έχει αποθηκευτεί στον server
                    const endTimeMsg = timerState.currentLanguage === 'en' ? 'TIME UP' : 'ΤΕΛΟΣ ΧΡΟΝΟΥ';
                    timerState.message = endTimeMsg;
                    ...
                    console.log(`✅ Server: Αυτόματο μήνυμα "${endTimeMsg}" στάλθηκε (γλώσσα: ${timerState.currentLanguage || 'el'})`);
                }
...
// =============================================================================
    ...
    socket.on('languageChange', (data) => {
        ...
        // ΚΡΙΣΙΜΟ: Αποθήκευση γλώσσας στο timerState για χρήση σε "TIME UP" μήνυμα
        timerState.currentLanguage = data.language;
        console.log(`💾 Server: Language stored as "${timerState.currentLanguage}"`);
